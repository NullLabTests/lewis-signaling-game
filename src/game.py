...
class LewisGame:
    ...
    def update(self, state, signal, action, reward):
        # simple REINFORCE-like update to increase prob of chosen moves if reward>0
        # Sender update (increase P(signal|state) when reward)
        if reward > 0:
            ...
            # small negative update to discourage this mapping
